...
All 8 milestones are implemented. 171 tests pass. The app builds and runs from a clean clone.
...
171 tests, all passing. Run with `swift test`.
...
**No UI tests.** The spec planned XCUITest smoke tests. There are none. The project builds through SwiftPM rather than an Xcode project, which XCUITest requires. Engine and store logic is covered by the 171 unit tests; view code is not covered.
